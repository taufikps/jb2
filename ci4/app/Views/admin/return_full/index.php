            ...
            <table class="table table-bordered table-hover bg-white align-middle mb-0 table-filter-table">
                ...
                <?php foreach ($rows as $r): ?>
                    <tr data-status="<?= isset($r['status']) ? strtolower($r['status']) : '' ?>">
                        ...
                        <?php endforeach; ?>
                        <td>
                            <a href="<?= site_url('admin/return-full/' . $r['id']) ?>" class="btn btn-sm btn-primary">Detail</a>
                            <form action="<?= site_url('admin/return-full/' . $r['id'] . '/resend') ?>" method="post" style="display:inline">
                                <button class="btn btn-sm btn-warning">Kirim ke D365</button>
                            </form>
                            <form action="<?= site_url('admin/return-full/' . $r['id'] . '/delete') ?>" method="post" style="display:inline" onsubmit="return confirm('Hapus data ini?')">
